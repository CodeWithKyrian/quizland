<x-layout title="Edit Post">
    <form id="edit-post-form" x-data="editPostData" enctype="multipart/form-data"
            ...
            <div class="flex justify-between items-center">
                <div class="flex items-center">
                    ...
                    <h2 x-show="editing === false"
                        class="form-input block text-sm text-black bg-transparent border-transparent dark:text-zinc-300"
                    ...
                    <div x-show="editing === true" class="text-base flex items-center">
                        ...
                        <input type="text" placeholder="Post Title" id="title" x-model="title" name="title"
                               x-on:blur="editing = false" required value="{{ $post->title }}"
            ...
            <div class="grid grid-cols-12 gap-6 text-zinc-700">
                <div class="col-span-6 px-5 py-2">
                    ...
                    <div class="text-sm py-2">
                        ...
                        <x-tag-input name="tags" :items="$post->tags->pluck('name')"
                                     :suggestions="$tags->pluck('name')"/>
                        @error('tags')
                        <div class="text-sm text-red-500">{{ $message }}</div>
                        @enderror
                    </div>
                            ...
                            <div class="flex items-center justify-center w-full mx-auto pb-[56.25%] relative mt-2">
                                <div class="absolute inset-0 overflow-hidden">
                                    <label for="thumbnail"
                                        ...
                                        <img id="preview" class="absolute object-cover w-full" alt=""
                                             src="{{$post->thumbnail}}">
                ...
                <div class="hidden">
                    ...
                    <label>
                        <input type="text" name="published_at" value="{{$post->published_at?->format('Y-m-d H:i:s')}}">
                    </label>
                </div>
            </div>
        </div>
    ...
    @push('scripts')
        ...
        <script>
            const editPostData = {
                ...
                saveDraft() {
                    document.querySelector('input[name="shouldPublish"]').checked = false;
                    document.querySelector('input[name="published_at"]').value = null;
                    document.querySelector('#edit-post-form').submit();
                },
                publishPost() {
